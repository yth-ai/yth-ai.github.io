...
2024 年的一项重要研究《How to Train Long-Context Language Models (Effectively)》（ACL 2025）发现了两个关键结论：
...
def design_length_curriculum(
    target_max_length: int = 131072,  # 128K
    num_stages: int = 5,
    tokens_per_stage: int = 100_000_000_000,  # 100B per stage
) -> List[dict]:
    """
    设计渐进式长度课程
    ...
    for i in range(num_stages):
        ...
        stage = {
            ...
            "length_distribution": {
                "short (< 4K)": max(0.30, 0.60 - i * 0.08),
                f"medium (4K-{stage_min//1024}K)": 0.30,
                f"long ({stage_min//1024}K-{stage_max//1024}K)": min(0.40, 0.10 + i * 0.08),
            },
            "rope_base": int(500000 * (2 ** i)),  # 逐步增大 RoPE base
        }
        stages.append(stage)
...
# 设计从 8K 到 128K 的课程
stages = design_length_curriculum(target_max_length=131072, num_stages=5)
for s in stages:
    print(f"阶段 {s['stage']}: max_len={s['max_context_length']//1024}K, "
          f"RoPE base={s['rope_base']:,}")
```
...
> - 2026："合成数据是有价值的工具，但必须在质量控制和混合比例上谨慎"
...
**在中训练中的实践意义**：
- **永远不要用 100% 合成数据做中训练**
- **混入真实数据是必须的**——真实数据维持了分布的多样性
- **推荐的合成数据比例**：10-30%，具体取决于领域和目标
...
1. **长上下文数据**：代码仓库和书籍是最佳来源；必须与短文档混合；渐进式扩展上下文长度
2. **代码数据**：质量分层是关键（Tier 1-4）；仓库级代码优于文件级；不要忘记代码相关的自然语言
3. **数学数据**：LaTeX 完整性是基本要求；合成数学题需要严格验证；难度分层很重要
4. **领域数据**：知识密度是核心指标；领域数据占比不宜超过 60%；通用回放是防止遗忘的必须
5. **合成数据**：是有价值的工具但不是银弹；混合比例建议 10-30%；Model Collapse 风险真实存在
6. **质量控制**：三层过滤（格式→模型打分→多样性）；人工抽检不可省略
