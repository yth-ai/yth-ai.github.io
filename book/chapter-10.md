...
> **💡 反直觉发现**
>
> 回放数据不需要和预训练数据一模一样。实际上，**用预训练数据的高质量子集做回放效果更好**——因为回放的目标不是"重现预训练"，而是"保持关键能力"。高质量子集在更少的 token 中包含了更密集的知识信号。
...
一种越来越流行的做法是在中训练中使用参数高效微调（PEFT）方法：
...
LoRA 中训练：
  ├── 优点：只更新低秩矩阵，主干参数不变 → 遗忘小
  ├── 缺点：学习能力受限，不适合需要大幅调整的场景
...
| 2024 | "系统化的遗忘研究" | ACL/EMNLP 上多篇论文量化遗忘 |
| 2025 | "遗忘是可预测和可控的" | 综合运用回放+权重平均+监控 |
| 2026 | "动态配比+实时监控成为标配" | 遗忘管理成为中训练基础设施的一部分 |
...
2. **遗忘是选择性的**：低频知识和非目标语言最脆弱
3. **回放是最有效的防控手段**：推荐回放比例 20-40%
4. **学习率控制是第二道防线**：中训练 lr 应为预训练峰值的 1/5 到 1/3
5. **权重平均是零成本的补救方案**：能恢复 50-80% 的遗忘
6. **实时监控不可或缺**：发现越早，修复成本越低
7. **数据配比可以在训练中途调整**——不需要完全重来
8. **分别监控不同语言和不同能力维度的遗忘**
