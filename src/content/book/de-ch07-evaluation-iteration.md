---
title: "预训练数据的评估与迭代"
description: "数据质量评估、统计可视化、下游任务验证与迭代闭环"
date: 2026-03-21
...
### 数据质量对 Scaling Laws 的量化影响

"数据不好会影响模型表现"是个直觉。但具体影响多大？有没有定量模型？直到最近，这个问题才有了严肃的回答。

[ACL 2025 的一项大规模实证研究](https://aclanthology.org/2025.acl-long.1163/)分析了 400 多个模型的训练数据，揭示了一个关键现象——**次级缩放**（sub-optimal scaling）：当训练数据中冗余信息（高数据密度）过多时，增加数据量或模型规模带来的性能提升会显著减速，偏离经典 Scaling Law 预测的幂律曲线。

这给了我们一个有用的心智模型：

- **经典 Scaling Law** 假设数据是"足够好的"——只要加量就能提升
- **次级缩放定律** 修正了这个假设——数据冗余会让 scaling 变"钝"，你以为加了 2 倍数据应该提升 X%，实际只提升了 0.6X%
- **实践含义**：在盯着"数据量够不够"之前，先检查"数据重复度高不高"。7.4 节的重复率统计不只是监控指标，而是直接关系到 scaling 效率的核心变量

这也解释了为什么去重（第 4 章）在整个数据管线中的地位如此关键——它不只是"清理噪声"，而是在维护 Scaling Law 的有效性。
...
前面提到需要建立"多维度的评估体系"。但一个自然的问题是：当你有文本复杂性、信息量、格式规范性、教育价值等七八个质量维度的分数时，如何把它们合成一个决策？

最朴素的做法是设阈值——每个维度都过了就留下。但这要么太严（所有维度都高的数据很少），要么太松（只要有一个维度高就留下）。实际操作中，大量时间花在了调多个阈值的组合上。

[FIRE（Flexible Integration of Data Quality Ratings）](https://arxiv.org/abs/2502.00761)在 EMNLP 2025 上提出了一种更系统的方案：将多个异构质量评级器对齐到统一的特征空间，通过渐进式数据选择进行迭代精炼。实验表明，FIRE 用不到 37.5% 的数据量即可达到随机选择全量数据的同等性能。

这代表了数据评估的一个范式升级——从"在 dashboard 上看多个指标然后拍脑袋"走向"让框架自动学习多信号的最优融合权重"。后续 7.7 节讨论的监控系统可以采集各维度指标，而 FIRE 这类框架负责将指标转化为数据选择决策。
...
**代理模型的下一步：从同构缩小到影响力保持**

上述策略都是在"用小模型近似大模型"的框架内打补丁。[IProX（ICLR 2026）](https://arxiv.org/abs/2602.17835)提出了一个更根本的思路：问题不在于代理模型太小，而在于简单缩小的小模型没有保持与大模型一致的**数据偏好方向**。

IProX 的两阶段框架：
1. 通过影响力保持的低秩近似压缩目标模型——不是训练一个新的小模型，而是从大模型本身出发，用低秩分解保留梯度影响力方向
2. 通过模型梯度和输出对齐进一步精炼代理

核心区别在于：传统代理是"同架构缩小"（比如从 70B 缩到 400M），参数量变了，数据偏好也跟着变了。IProX 的代理是"影响力保持压缩"——参数量变小，但对数据的梯度响应方向尽量与大模型一致。

需要注意的是，IProX 目前主要在 SFT 数据选择上得到了验证，是否能直接迁移到预训练数据消融场景还有待更多实验。但它指明了一个方向：代理模型的可靠性问题，可能需要在"如何构建代理"而非"如何使用代理"的层面上解决。
...
前面讨论的消融维度都是"改管线参数"——调阈值、换提取器、改配比。但还有一个正交的维度往往被忽视：**在相同管线输出中，用不同策略选择训练子集**。

[一项针对 19 种数据采样方法的大规模对比研究](https://arxiv.org/abs/2402.09668)（ICLR 2026）发现：

- **AskLLM**（用指令微调 LLM 直接评估每条训练样本的质量）在质量维度表现最优
- **Density Sampling**（基于数据分布建模选择多样性样本）在覆盖维度表现最优
- 两者组合，可以用 10-30% 的数据达到全量训练的同等性能

这给消融实验设计带来了一个新视角：除了问"哪个管线参数最好"，还可以问"同一管线的输出数据中，用什么策略选子集最高效"。后者的实验成本更低——不需要重新跑数据管线，只需要在已有数据上做采样。

需要注意的是，该研究的验证规模主要在 1-7B 模型上，对于更大规模模型是否同样成立还需要更多实验。但 10-30% 的数据效率数字，至少表明在消融实验的"快速筛选"阶段，数据子集选择是一个值得尝试的低成本方向。
...
上面列出的 benchmark 都有一个共同的软肋：它们是**静态的**。题目公开之后，就有可能通过各种渠道进入训练数据——直接泄露、网页引用、数据集整合，甚至模型生成的训练数据中包含对这些题目的讨论。

[LiveBench](https://livebench.ai)（ICLR 2025）代表了一种从根本上解决这个问题的范式：**与其在静态 benchmark 上费力做去污染修补，不如用不可能被提前泄露的题目**。LiveBench 的核心设计：

- **定期发布全新题目**，旧题目退役，确保测试集的"新鲜度"
- **所有题目都有可验证的客观标准答案**，不需要 LLM-as-Judge 这类可能引入偏差的评判方式
- 覆盖 6 大类 18 项任务，即使最强模型也很少超过 65% 准确率

对于数据工程团队来说，动态评测的价值在于：当你在静态 benchmark 上的改进很难排除污染因素时，可以用 LiveBench 这类平台做交叉验证。如果改进在动态评测上也成立，那就是真实的数据质量提升。
...
上面的 n-gram 检测方法是当前最广泛使用的去污染方案，GPT-3 和 Llama 都采用了类似思路。但 2025-2026 年的研究揭示了三个根本性的局限：

**1. RL 训练会系统性掩盖污染信号**

[ICLR 2026 的一项研究](https://arxiv.org/abs/2510.02386)发现了一个令人警醒的现象：即使在 SFT 阶段注入了明确的 benchmark 污染，经过短暂的 GRPO/PPO 训练后，现有的所有检测方法效果都会降到接近随机猜测。根本原因在于 PPO 风格的重要性采样和剪切目标天然具有"洗白"污染信号的能力——RL 优化的是策略分布，会抹平 SFT 阶段记忆的特定样本痕迹。

这意味着对于使用了 RL 训练的推理模型（如 o1、DeepSeek-R1 等），传统的去污染检测方法可能完全失效。

**2. 从二元判定到连续风险量化**

传统检测给出的是"污染/不污染"的二元结论。[DCR（Data Contamination Risk）](https://arxiv.org/abs/2507.11405)在 EMNLP 2025 上提出了一种更细粒度的方案：在语义、信息、数据、标签四个级别检测污染风险，通过模糊推理系统合成统一的 DCR Factor。经调整后的准确率与未污染基准线误差控制在 4% 以内，已在 9 个 LLM（0.5B-72B）上验证通过。

DCR 的价值不在于替代 n-gram 检测，而在于提供了一种"风险调整后分数"的思路——不是简单地去掉疑似污染的样本，而是根据污染风险对评估分数做折扣。

**3. 从数据比对到模型内视**

另一个互补方向来自[NA-PDD（EMNLP 2025）](https://arxiv.org/abs/2507.16414)：通过分析模型内部神经元激活模式的差异，判断某条数据是否出现在预训练集中。这种"模型内视"方法不需要访问训练数据或重新训练模型，对于文本被改写、变体等传统 n-gram 方法难以捕捉的间接污染尤其有效。

**实践建议**：在当前阶段，n-gram 检测仍然是必要的基础设施——它能捕捉大部分显式污染。但不应将其视为充分手段。对于重要的评估结论，建议采用多层次验证：n-gram 检测 → DCR 风险量化 → 与动态评测平台（下文讨论）交叉验证。
...
| 2025 | "多信号融合与动态评测" | FIRE 用 37.5% 数据达到同等性能；LiveBench 动态评测范式兴起；DCR 四级污染风险量化 |
| 2026 | "推理模型时代的评估危机" | ICLR 2026 揭示 RL 训练系统性掩盖污染信号；IProX 影响力保持代理；AskLLM+Density Sampling 数据高效选择 |
...
> 2. **数据冗余会导致次级缩放**——高数据密度让 Scaling Law 变"钝"，去重不只是清理噪声，而是维护 scaling 效率
> 3. **多质量信号应该融合而非并列**——FIRE 等框架能用不到 37.5% 的数据达到同等性能
> 4. **小模型代理法是当前最实用的评估方案**，但有可靠性限制；影响力保持代理（IProX）可能是下一步方向
> 5. **消融实验要分层设计**，除了调管线参数，还可以尝试数据子集选择策略
> 6. **数据分布 dashboard 是必需品**，而非锦上添花
> 7. **Benchmark 去污染是评估的前提**，但在推理模型时代面临系统性失效——需要多层次验证
> 8. **动态评测正在成为静态 benchmark 的重要补充**——用不可能被提前泄露的题目从源头解决污染问题
> 9. **数据迭代是持续闭环**，不是一锤子买卖
> 10. **版本管理和持续监控**是数据管线工程化的基础
